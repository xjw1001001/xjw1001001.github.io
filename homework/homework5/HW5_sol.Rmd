...
Let N denotes the number of person that IQ is over 120 for these 4 people, then $N \sim Binomial(4,0.0918)$
...
As the sample size grows, we expect the sample mean to converge to the theoretical mean of the distribution, which in this case is 900. This is because the Law of Large Numbers tells us that as the sample size increases, the sample mean becomes a more accurate estimate of the population mean.

```{r}
num_attend <- rbinom(10000, 1, 0.6)
mean(num_attend)
```
...
```{r}
ggarrange(p1,p2,labels = c("Mean","Gender"), ncol = 2, nrow = 1)
```
...
population_2 = population_2 |>
  mutate(weight = ((sex == "Male")*1/6 + (sex == "Female") * 5/6) )
```


5\. Now we draw 300 samples **without replacement** with the given weight from part 4. (Hint: `dataset_to_sample |> sample_n(size = ?, replace = ?, weight = ?)` The code will sample the dataset_to_sample by row for the given arguments.) 

```{r}
sample = population_2 |> sample_n(size = 300, replace = FALSE, weight  = weight)
```
...
result_mean = rep(0, n)
result_female_count = rep(0, n)

for(i in 1:n){
  sample1 = population_2 |> sample_n(size = 300, replace = FALSE, weight  = weight)
  result_mean[i] = mean(sample1$length)
  result_female_count[i] = sum((sample1$sex =="Female"))
}
result_df = data.frame(
  mean = result_mean,
  count = result_female_count
)
```
...
```{r}
p1 = result_df |> ggplot(aes(x = count)) + 
  geom_histogram() +
  geom_vline(aes(xintercept = 300 * (3000)/(2000+3000)),
             size = 2, color = "blue")
p2 = result_df |> ggplot(aes(x = mean)) + 
  geom_histogram() +
  geom_vline(aes(xintercept = mean(population_2$length)),
             size = 2, color = "blue")

ggarrange(p1, p2, labels = c("Female count","Mean length"),
         ncol = 2, nrow = 1)
```
