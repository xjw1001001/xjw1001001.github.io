...
## Problem 1: combination (15 points)
...
1\. How many ways can she pick 4 vegetables toppings?
```{r}
choose(7,4)
```
...
```{r}
choose(3,1)*choose(4,2)*choose(7,1)
```
...
```{r}
denominator = choose(7+4+3,4)
zero_protein = choose(7,2)*choose(4,2)*choose(3,0)
two_protein = choose(7,1)*choose(4,1)*choose(3,2)
numerator = zero_protein+two_protein
...
Outliers: observations that are unusually far from the rest of the data. Sample mean is sensitive to outliers because it relies on the addition property of all the observations in the data. Sample standard deviation is sensitive to outliers because it relies on the addition of all the squared difference between observations and sample mean, which can be extremely large for the outliers.
...
```{r}
a2_subset = a2[!outliers]
n = length(a2_subset)
sample_mean = sum(a2_subset)/n
sample_sd = sqrt(sum((a2_subset-sample_mean)^2)/(n-1))
sample_mean
sample_sd
```

## Problem 3: COVID-19 data analysis (65+2 points)
...
4\. Create a scatterplot using `ggplot` representing `pop_density` vs. various variables: per100k and newper100k for each state on 2021-04-28, using `cv_states_20210428`. Label points by `abb` and size points by `population`, color by the naive_CFR. Detailed instruction below: (16+2 points)
...
6\. Create a plot of daily cases similar to the one at the top of [this page](https://www.nytimes.com/interactive/2020/us/coronavirus-us-cases.html) for the selected states. Detailed instruction below: (14 points)
...
7\. Create a map to visualize the `naive_CFR` by `state` on Jan 1, 2021 and Jan 1, 2023. Color the boundary white with `colour = "white"`. Fill the states by the `naive_CFR`. Describe the difference in the pattern of the CFR. (5 points)
