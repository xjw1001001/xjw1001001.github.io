...
subtitle: "<br><br> STA 032: Gateway to data science Lecture 19"
...
library(renderthis)
to_pdf(from = "lecture19.html",complex_slides = TRUE)
```
...
- **Set up**: $X_i$ are independent and identically distributed with population mean $\mu$ and variance, $\sigma^2$. We are interested in a confidence interval for the unknown population parameter $\mu$. We use the sample mean, $\overline{X}$, constructed from a sample of size $n$, as an estimator for the population mean. Assume $n$ is large. And we also assume population variance $\sigma^2$ is known.
